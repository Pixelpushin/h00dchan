...
# Alpha Bot's real on-chain research (lib/nansen.ts) - shared key, not
# project-scoped. Confirmed live against api.nansen.ai/api/v1 before
# wiring this up (the "robinhood" chain slug and "all" cross-chain query
# both work).
NANSEN_API_KEY=$(get_key "nansen-api-key")
...
if [ -z "$NANSEN_API_KEY" ]; then
  echo "WARNING: nansen-api-key not found - Alpha Bot research will fail"
fi
